...
public abstract class ActionBarSherlock {
    ...
    public static void registerImplementation(Class<? extends ActionBarSherlock> implementationClass) {
        if (!implementationClass.isAnnotationPresent(Implementation.class)) {
            ...
            if (SherlockConfig.DEBUG) Log.w(TAG, "Class " + implementationClass.getSimpleName() + " already registered");
            return;
        }

        Implementation impl = implementationClass.getAnnotation(Implementation.class);
        if (SherlockConfig.DEBUG) Log.i(TAG, "Registering " + implementationClass.getSimpleName() + " with qualifier " + impl);
    ...
    public static ActionBarSherlock wrap(Activity activity, int flags) {
        ...
        if (SherlockConfig.DEBUG) Log.i(TAG, "Using implementation: " + impl.getSimpleName());
    ...
    protected ActionBarSherlock(Activity activity, int flags) {
        if (SherlockConfig.DEBUG) Log.d(TAG, "[<ctor>] activity: " + activity + ", flags: " + flags);
    ...
    protected final boolean callbackCreateOptionsMenu(Menu menu) {
        if (SherlockConfig.DEBUG) Log.d(TAG, "[callbackCreateOptionsMenu] menu: " + menu);
        ...
        if (SherlockConfig.DEBUG) Log.d(TAG, "[callbackCreateOptionsMenu] returning " + result);
        return result;
    }
    ...
    protected final boolean callbackPrepareOptionsMenu(Menu menu) {
        if (SherlockConfig.DEBUG) Log.d(TAG, "[callbackPrepareOptionsMenu] menu: " + menu);
        ...
        if (SherlockConfig.DEBUG) Log.d(TAG, "[callbackPrepareOptionsMenu] returning " + result);
        return result;
    }
    ...
    protected final boolean callbackOptionsItemSelected(MenuItem item) {
        if (SherlockConfig.DEBUG) Log.d(TAG, "[callbackOptionsItemSelected] item: " + item.getTitleCondensed());
        ...
        if (SherlockConfig.DEBUG) Log.d(TAG, "[callbackOptionsItemSelected] returning " + result);
        return result;
    }
    ...
    public void setContentView(View view) {
        if (SherlockConfig.DEBUG) Log.d(TAG, "[setContentView] view: " + view);
    ...
    public void setTitle(int resId) {
        if (SherlockConfig.DEBUG) Log.d(TAG, "[setTitle] resId: " + resId);
    ...
    public MenuInflater getMenuInflater() {
        if (SherlockConfig.DEBUG) Log.d(TAG, "[getMenuInflater]");
